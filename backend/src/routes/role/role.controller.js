...
const { assignRole, createRole, deleteRole, getRole, listRoles, removeRole, updateRole } = require("./role.service");

const roleAssign = asyncHandler(async (req, res) => {
  const { role_id, user_id } = req.body;
  await assignRole(user_id, role_id);
...
const roleRemove = asyncHandler(async (req, res) => {
  const { role_id, user_id } = req.body;
  await removeRole(user_id, role_id);
